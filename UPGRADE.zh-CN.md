...
- 无破坏性变更。
- 不需要手动执行数据库迁移。
- 现有 Site Key、Secret Key、页面开关、主题、对齐方式和自定义选择器会保留。
- 本版本修复了 WHMCS 后台保存客户资料时可能被 Turnstile 注册验证错误拦截的问题。
- 上传后，请测试 WHMCS 后台客户资料保存；如果启用了注册页保护，也请测试前台注册页面。

### 从 1.4.7 升级到 1.4.8

- 无破坏性变更。
- 不需要手动执行数据库迁移。
- 现有密钥、页面开关、主题、对齐方式和自定义选择器会保留。
- 保存设置时，`Secret Key` 字段留空会保留已保存的密钥。
- 插件后台页面现在包含 GitHub 仓库入口和浏览器侧更新提示；该后台显示不需要服务端迁移。
- 如果客户区小组件上传后没有立即更新，请先清理 WHMCS 模板缓存和浏览器缓存再测试。
